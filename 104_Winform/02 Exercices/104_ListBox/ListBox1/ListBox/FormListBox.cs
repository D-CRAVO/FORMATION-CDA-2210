...
namespace ListBox
{
    public partial class FormListBox : Form
    {
        ...
        private void buttonAjout_Click(object sender, EventArgs e)
        {
            ...
            //}
            bool ok = false;
            string saisieUtilisateurSansEspacesInutiles = textBoxNouvelElement.Text.Trim();
            /*if (saisieUtilisateurSansEspacesInutiles == "")
                return;*/
            if (!Controles.ControleTexteSaisi(textBoxNouvelElement.Text))
                return;
            foreach (var item in listBoxListe.Items)
            {
                if (saisieUtilisateurSansEspacesInutiles == item.ToString())
                {
                    ok = true;
                }
            }
            if (!ok)
            {
                ...
                listBoxListe.Items.Add(saisieUtilisateurSansEspacesInutiles);
