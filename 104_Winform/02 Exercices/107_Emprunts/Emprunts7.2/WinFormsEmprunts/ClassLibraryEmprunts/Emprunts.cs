...
namespace ClassLibraryEmprunts
{
    public class Emprunts
    {
        ...
        public int Periodicite { get { return periodicite; } set { periodicite = value; } }
        ...
        public Emprunts(string? _nomClient, uint _capitalEmprunte, int _nbMois, int _periodicite, double _tauxAnnuel)
        {
        ...
        /// </summary>
        public Emprunts() : this (string.Empty, 1, 1, 1, 0.07) { }
        ...
        {
            if (periodicite == (int)enumPeriodicite.mensuelle)
            {
                return nbMois / (int)enumPeriodicite.mensuelle;
            }
            else if (periodicite == (int)enumPeriodicite.bimestrielle)
            {
                return nbMois / (int)enumPeriodicite.bimestrielle;
            }
            else if (periodicite == (int)enumPeriodicite.trimestrielle)
            {
                return nbMois / (int)enumPeriodicite.trimestrielle;
            }
            else if (periodicite == (int)enumPeriodicite.semestrielle)
            {
                return nbMois / (int)enumPeriodicite.semestrielle;
            }
            else
            {
                return nbMois / (int)enumPeriodicite.annuelle;
            }
        }
